...
- 发布前总控使用 `scripts/validate-release.ps1`；`dev` 只做本地开发验收和源码预览，不生成公开 release artifact，`stable` 必须带 `-UserTested` 且完成 release build、public audit、启动检查和适用的 updater/feed 检查；v0.2.1 Windows 公开 Release 只保留可自动更新的 setup 安装包、`latest.json` 和 `SHA256SUMS.txt`，portable packaging 仅作为手工/本地包能力保留；macOS Apple Silicon 可补充 unsigned/ad-hoc `.dmg` 和 updater `.app.tar.gz`，unsigned/notarization 说明只写在 docs 和 GitHub Release notes，不写进软件 UI。
