...
- `latest.json` contains the signed platform URL and signature for each approved stable target, currently `windows-x86_64`, unsigned/ad-hoc `darwin-aarch64`, and Ubuntu/Debian `linux-x86_64`.
...
The Linux workflow can build x86_64 AppImage and `.deb` artifacts on a GitHub-hosted Ubuntu runner:
...
- The uploaded CI artifact uses the current package version, for example `codexhub-linux-v<version>-x86_64-release`.
- The GitHub Release includes `CodexHub_<version>_amd64.AppImage` and `CodexHub_<version>_amd64.deb` only after owner approval.
- The merged feed includes `linux-x86_64` only when the signed AppImage updater artifact is approved.
