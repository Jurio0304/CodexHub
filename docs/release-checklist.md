...
- Portable packaging remains manual/local for now; v0.2.5 Windows public Release keeps the updater-enabled setup installer as the only Windows app package.

## macOS Release Artifact

The macOS workflow can build unsigned `.app`, `.dmg`, and updater `.app.tar.gz` artifacts on a GitHub-hosted macOS runner. The v0.2.5 public GitHub Release includes unsigned Apple Silicon macOS assets:
