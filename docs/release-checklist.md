...
- `latest.json` contains the signed platform URL and signature for each approved updater target, currently `windows-x86_64` and unsigned/ad-hoc `darwin-aarch64`. Linux `.deb` packages are manual install artifacts and should not add a Linux platform key.
...
- Portable packaging remains manual/local for now; v0.4.3 Windows public Release keeps the updater-enabled setup installer as the only Windows app package.

## macOS Release Artifact

The macOS workflow can build unsigned `.app`, `.dmg`, and updater `.app.tar.gz` artifacts on a GitHub-hosted macOS runner. The v0.4.3 public GitHub Release includes unsigned Apple Silicon macOS assets:

```text
.github/workflows/build-macos-release.yml
```

The v0.4.3 macOS artifact still requires real Mac validation. For each future macOS public artifact, verify:
