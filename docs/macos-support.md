...
Status: macOS release-build support is merged into `master`. The v0.4.0 macOS public artifact is unsigned/ad-hoc and still requires real Mac validation for this release.
...
The v0.4.0 public macOS artifact still needs real-device validation for:
...
- v0.4.0 still requires real Mac validation for GUI launch, Gatekeeper behavior, `.app`/`.dmg` packaging, and Codex App handoff.
