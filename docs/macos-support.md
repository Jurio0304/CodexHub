...
Status: preview work on `feature/macos-support`. This branch is not merged to `master` and is not a formal release.

CodexHub remains conservative: it writes CodexHub-managed SSH blocks to the user's SSH config, avoids Codex App private state, and keeps remote Codex work on the existing SSH/SFTP path. macOS support is currently buildable and mock-testable from Windows, but GUI behavior and installed app behavior require a real Mac.
...
The preview workflow is `.github/workflows/build-macos-preview.yml`.

To download a test build:

1. Open the GitHub repository Actions tab.
2. Run or open `Build macOS Preview`.
3. Wait for the `macOS unsigned preview` job to finish.
4. Download the `codexhub-macos-unsigned-preview` artifact.
5. Extract the artifact on a real Mac and test the `.app` or `.dmg`.

The workflow uploads temporary artifacts only. It does not create a GitHub Release, does not tag a release, and does not notarize the app. The build uses ad-hoc signing (`APPLE_SIGNING_IDENTITY=-`) only as a preview compatibility measure.

## Gatekeeper Notes

This preview is not notarized. On a real Mac, Gatekeeper may block the app on first launch. For testing, use Finder's Open action or the system Privacy & Security prompt to allow the app after you confirm the artifact came from the expected GitHub Actions run.

Do not distribute this artifact as a release build.
...
- The preview workflow does not publish a release.
- Windows is still the validated production release platform until this branch is tested on a real Mac.
