...
- `CODEXHUB_STABLE_UPDATER_PUBKEY`: Tauri updater public key injected at build time. The release scripts normalize either the direct public key line or a generated minisign `.pub` file value to the public key string expected by Tauri; do not commit private signing material.
