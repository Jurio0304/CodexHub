...
- The Windows updater feed is `latest.json` and currently contains only the `windows-x86_64` platform entry.
...
The Windows release workflow only uploads updater assets to an existing GitHub Release when manually dispatched with `upload_to_release=true`.
