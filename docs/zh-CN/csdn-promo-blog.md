...
Windows 版本支持托盘图标；macOS 版本面向 Apple Silicon 提供 `.dmg` 安装包，v0.4.2 本轮仍需要真实 Mac 验证；当前 macOS 构建仍是 unsigned/ad-hoc，首次打开可能需要通过系统安全设置手动允许。日常使用请优先从 GitHub Releases 下载可信构建。
