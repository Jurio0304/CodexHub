...
- Windows：下载并运行 `CodexHub_0.4.0_x64-setup.exe`。
- macOS Apple Silicon：下载 `CodexHub_0.4.0_aarch64.dmg`，打开后将 `CodexHub.app` 移入 Applications。v0.4.0 macOS 资产仍为 unsigned/ad-hoc，且本轮仍需要真实 Mac 验证；首次打开时可能需要通过 Control-click > Open 或 Privacy & Security 手动允许。只信任从本仓库 Release 页面下载的文件。
