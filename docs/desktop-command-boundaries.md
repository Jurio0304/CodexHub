...
- The frontend never requests stored API key values and never offers password/API-key reveal controls.
- Passwords and API keys may cross IPC only for the explicit operation that needs them. Command errors and logs never include command arguments.
